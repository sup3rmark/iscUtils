...
#>
    ...
    process {
        ...
        $rolesData = @()
        do {
            $url = "$baseURL&offset=$($rolesData.count)&limit=$Limit"
            ...
            Write-Verbose "Retrieved $($rolesData.count) of $($responseHeaders.'X-Total-Count') records."
        } while ($rolesData.count -ne $($responseHeaders.'X-Total-Count'))
